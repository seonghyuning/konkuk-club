 ...
 * 활동중인 동아리 관리
 */
class ActiveManagerActivity : AppCompatActivity() {
    lateinit var binding: ActivityManagerBinding
    private lateinit var database: DatabaseReference
    lateinit var adapter: ManagerAdapter
    private var clubData:ArrayList<Club> = ArrayList()  //불러온 동아리 data
    private var searchedClubData:ArrayList<Club> = ArrayList()  //검색된 동아리 data
    ...
    private fun initLayout() {
        ...
        //제목 변경
        binding.clubStateText.text = "활동중인 동아리"
        ...
        binding.moreSpinner.onItemSelectedListener = object : AdapterView.OnItemSelectedListener{
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                if(selectedManager == "승인대기") {
                    //승인 대기중인 상태의 동아리로 이동
                    val intent = Intent(this@ActiveManagerActivity, ApprovalManagerActivity::class.java)
                    ...
                    //비활성화 상태인 동아리로 이동
                    val intent = Intent(this@ActiveManagerActivity, InactiveManagerActivity::class.java)
                    intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION)
                    startActivity(intent)
                    overridePendingTransition(0, 0)
                    finish()
                    ...
                    //신고된 상태의 동아리로 이동
                    val intent = Intent(this@ActiveManagerActivity, ReportManagerActivity::class.java)
                    intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION)
                    startActivity(intent)
                    overridePendingTransition(0, 0)
                    finish()
            ...
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    for (postSnapshot in snapshot.children) {
                        ...
                        if(club!!.state == "ACTIVE") {  //활성화된 동아리의 경우에만
    ...
    private fun initRecyclerView() {
        ...
        adapter.itemClickListener=object : ManagerAdapter.OnItemClickListener {
            override fun OnItemClick(data: Club, position: Int) {
                //동아리 상세로 이동
                val intent = Intent(this@ActiveManagerActivity, ClubInfoActivity::class.java)
